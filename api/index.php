...
$db = new DB("127.0.0.1", "SocialNetwork", "root", "root");
                        ...
                        if (strlen($username) >= 3 && strlen($username) <= 32) {
                                ...
                                if (preg_match('/[a-zA-Z0-9_]+/', $username)) {
                                        ...
                                        if (!$db->query('SELECT email FROM users WHERE email=:email', array(':email'=>$email))) {
                                                ...
                                                $db->query('INSERT INTO users (id, username, password, email, verified) VALUES (NULL, :username, :password, :email, \'0\')', array(':username'=>$username, ':password'=>password_hash($password, PASSWORD_BCRYPT), ':email'=>$email));
