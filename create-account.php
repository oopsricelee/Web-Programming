...
<body>  
    ...
    <div class="login-clean">
        <form method="post">
            ...
            <!-- <div class="form-group photo"> -->
                <!-- <img width='145' src="http://s3.amazonaws.com/37assets/svn/765-default-avatar.png"> -->
                <!-- <input class="hidden" type="file" name="postimg"> -->
            <!-- </div> -->
            ...
            <div class="form-group">
                <button class="btn btn-primary btn-block" id="ca" type="button" name="createaccount" data-bs-hover-animate="shake">Create Account</button>
            </div>
            <a href="#" class="forgot">Already got an account? Click here!</a></form>
    ...
    <script type="text/javascript">
        ...
        $('#ca').click(function(event) {
                // var idx = $('#avatar').attr('src').indexof('base64');
                // var img = $('#avatar').attr('src').substring(idx);
                // alert(idx);
                // alert($('#avatar').attr('src').substring($('#avatar').attr('src').indexOf('base64')+7));
                // var formData = new FormData();
                //         formData.append('username', $('#username').val());
                //         formData.append('email', $('#email').val());
                //         formData.append('password', $('#password').val());
                        // formData.append('profileimg', $('input[type=file]')[0].files[0]); 
                // $_POST['username'] = $("#username").val();
            $.ajax({
                ...
                success: function(r) {
                        // alert(r);
                        // alert($('#avatar').attr('src').indexOf('base64'));
                        // alert($('#avatar').attr('src').substring(23));
